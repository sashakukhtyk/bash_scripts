...
copy_ssh_key_to_remote() {
  ...
  ssh "$REMOTE_HOST" "mkdir -m700 $REMOTE_SSH_DIR && mv /tmp/id_ed25519_to_hostname.pub $REMOTE_SSH_DIR/authorized_keys && chown -R $SSH_USER:$SSH_USER $REMOTE_SSH_DIR"
}

# 4. Setup SSH config on local host for port forwarding
setup_ssh_config() {
  sudo mkdir -p /etc/ssh/ssh_config.d
  sudo bash -c "cat > /etc/ssh/ssh_config.d/sshfwd-to-hostname.conf <<EOF
...
ExecStart=$AUTOSSH_CMD -M 0 -TN -q -o 'ServerAliveInterval 60' -o 'ServerAliveCountMax 3' sshfwd-to-hostname
ExecStop=/usr/bin/pkill -9 -u $SSH_USER # kill all processes owned by $SSH_USER
...
setup_sshfs_mount() {
  ...
  sudo bash -c "cat > /etc/systemd/system/mount-remote-fs.mount <<EOF
[Unit]
...
Type=fuse.sshfs
Options=_netdev,allow_other,port=22,default_permissions,IdentityFile=$REMOTE_SSH_KEY,reconnect,ServerAliveInterval=30,ServerAliveCountMax=5,x-systemd.automount,uid=33,gid=33
TimeoutSec=60
